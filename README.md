...
When Dizzy Ticket Manager 1.10.0 or newer is active, choosing the buy-ticket option creates the reservation as `pending_payment`, reserves the table for 15 minutes, and sends the visitor to Mollie. A verified paid webhook changes the reservation to `confirmed` and sends the reservation confirmation. Failed or cancelled payments return to the reservation page with the form restored so the same reservation can retry payment without creating a duplicate. If payment is not completed within 15 minutes, the table becomes available again automatically.
