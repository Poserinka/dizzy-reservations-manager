...
final class ReservationService
{
    ...
    public function create(array $data): int
    {
        ...
        $capacity=$this->repository->capacity($occurrenceId);$status=$capacity>0&&$this->repository->reservedGuests($occurrenceId)+$guests>$capacity?'waitlisted':'pending';
    ...
    public function changeStatus(int $id,string $status): bool
    {
        ...
        if($status==='confirmed'){$capacity=$this->repository->capacity((int)$row['occurrence_id']);if($capacity>0&&$this->repository->reservedGuests((int)$row['occurrence_id'],$id)+(int)$row['guests']>$capacity) return false;}
